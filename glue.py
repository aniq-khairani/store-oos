...
# Redshift JDBC credentials
url = "jdbc:redshift://kkmart-redshift.cyk7xpzn3jbq.ap-southeast-1.redshift.amazonaws.com:5439/kkmart"
user = "admin"
password = "BZGUUzwush472)*"

# Get JDBC connection via JVM
conn = spark._sc._jvm.java.sql.DriverManager.getConnection(url, user, password)
stmt = conn.createStatement()

# Your SQL (MUST be a single string with no comments or newlines that confuse Redshift)

# ingest store nostock items
sql_delete = """ delete from transformed.store_nostock_items where last_updated::date = current_date; """

sql_insert = """ insert into transformed.store_nostock_items
(
    str_no, goo_no, goo_na, cla_no, cla_na, dcsup_no, dcsup_na, sup_no, sup_na, 
    sto_date, sto_qty, l_ord_date, ord_qty, auto_ord, first_ord, ord_factor, 
    orderable, sellable, roq, dist_type, dist_ctr, dc_stobal, last_grn_date, 
    grn_qty, last_updated
)
WITH 
store AS (
    SELECT * FROM (
...
item AS (
    SELECT * FROM (
        ...
        FROM source_current.mkfgoomi M
...
# Execute query
print("Executing Redshift SQL...")

# stmt.execute(sql_insert_exclude_item)
